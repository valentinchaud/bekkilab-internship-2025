...
module Session6.WordToVecSave (main) where
...
modelPath =  "data/sample_embedding2.params"
        ...
        let loopBatch startIdx m o
              ...
              | otherwise = do
                  let endIdx = min (startIdx + batchSize) numSamples
                      ...
                      linOut = Torch.NN.linear (Session6.WordToVecSave.linear m) embTensor
...
main = do
  ...
  let wordToIndex = wordToIndexFactory wordlst
      ...
      vectorsStr = map (\(bs, i) -> (decode $ B.unpack bs, i)) vectors
      (xTrainIdxs, yTrainIdxs) = convertPairsToIndices skipGrams vectors
  Bin.encodeFile wordLstPath vectorsStr
